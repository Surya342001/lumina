...
def send_agent_result(to_email: str, goal: str, result_text: str) -> bool:
    """
    Send AI agent search/recommendation result to user via email.
    Returns True on success, False on failure.
    """
    if not GMAIL_APP_PASS:
        print("[Email] GMAIL_APP_PASSWORD not set — skipping agent result email.")
        return False

    subject = "🤖 Nova Agent Found Results for You | Aurbis"
    html = f"""
<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><title>Nova Agent Results</title></head>
<body style="margin:0;padding:0;background:#F4F6FF;font-family:'Segoe UI',Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0">
    <tr>
      <td align="center" style="padding:32px 16px 0;">
        <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;background:linear-gradient(135deg,#0D0F1A,#1A1040);border-radius:16px 16px 0 0;">
          <tr>
            <td style="padding:28px 36px;">
              <div style="font-size:20px;font-weight:800;letter-spacing:0.15em;color:#fff;">AURBIS</div>
              <div style="font-size:11px;color:rgba(255,255,255,0.4);letter-spacing:0.25em;text-transform:uppercase;margin-top:4px;">NOVA AI AGENT</div>
            </td>
            <td align="right" style="padding:28px 36px;">
              <div style="background:rgba(99,102,241,0.3);border:1px solid rgba(99,102,241,0.5);border-radius:20px;padding:7px 16px;">
                <span style="color:#a5b4fc;font-size:12px;font-weight:700;">🤖 AGENT RESULT</span>
              </div>
            </td>
          </tr>
        </table>
    ...
    <tr>
      <td align="center" style="padding:0 16px 32px;">
        <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;background:#fff;border-radius:0 0 16px 16px;box-shadow:0 8px 32px rgba(0,0,0,0.08);">
          <tr>
            <td style="padding:32px 36px 20px;">
              <p style="margin:0 0 6px;font-size:12px;color:#9CA3AF;text-transform:uppercase;letter-spacing:0.1em;">Your Goal</p>
              <div style="background:#F8F9FF;border-left:4px solid #6C47FF;border-radius:0 8px 8px 0;padding:12px 16px;font-size:14px;color:#374151;font-style:italic;">"{goal}"</div>
            </td>
          </tr>
          <tr>
            <td style="padding:0 36px 32px;">
              <p style="margin:0 0 12px;font-size:13px;font-weight:700;color:#1E2235;">🎯 Nova's Recommendation</p>
              <div style="background:#F0FDF4;border:1px solid #BBF7D0;border-radius:12px;padding:20px 24px;font-size:14px;color:#166534;line-height:1.7;white-space:pre-wrap;">{result_text}</div>
            </td>
          </tr>
          <tr>
            <td style="padding:0 36px 32px;text-align:center;">
              <a href="http://localhost:5173" style="display:inline-block;background:linear-gradient(135deg,#6C47FF,#4B2FD4);color:#fff;font-size:14px;font-weight:700;padding:14px 32px;border-radius:10px;text-decoration:none;">Book on Aurbis →</a>
            </td>
          </tr>
          <tr>
            <td style="padding:0 36px 24px;text-align:center;">
              <p style="font-size:11px;color:#9CA3AF;">This result was generated by Nova, Aurbis AI Agent • aurbis.in</p>
            </td>
          </tr>
        </table>
      </td>
    ...
    msg["From"] = GMAIL_USER
    msg["To"] = to_email
    msg.attach(MIMEText(f"Your Goal: {goal}\n\nNova's Answer:\n{result_text}", "plain"))
    msg.attach(MIMEText(html, "html"))
    ...
    try:
        with smtplib.SMTP_SSL("smtp.gmail.com", 465) as server:
            server.login(GMAIL_USER, GMAIL_APP_PASS)
            server.sendmail(GMAIL_USER, to_email, msg.as_string())
        print(f"[Email] ✅ Agent result sent to {to_email}")
        return True
    except Exception as e:
        print(f"[Email] ❌ Agent result email failed: {e}")
        return False
